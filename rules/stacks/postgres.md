...
> ⛔ **스키마·데이터 이관 작업 자체는 사람이 수행한다. AI에게 맡기지 않는다.**
> 덤프/복원, 데이터 이전, 컷오버, 시퀀스 보정 같은 실행 작업은 이 룰의 대상이 아니다.
> 이 문서는 **이관 이후 애플리케이션 코드가 따르는 DB 레이어 규칙**이며, 이관 시 무엇이
> 깨지는지 사람이 판단할 때 쓰는 체크리스트다.
>
> **AI의 DB 접근은 읽기 전용 계정으로만 허용한다.** 스키마 조회·데이터 확인은 되지만
> 쓰기·DDL은 계정 권한 자체로 막는다. 지시가 아니라 권한이 경계선이어야 한다 —
> 문서로만 막으면 언젠가 뚫린다.
>
...
> 이 절은 **사람이 이관 절차를 설계할 때 보는 참고**다. 실행 주체는 사람이다(상단 경계 참조).
